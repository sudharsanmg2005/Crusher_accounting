...
const ConflictTable = ({
  ...
  selectedConflicts,
  onSelectConflict,
  onSelectAllConflicts
}) => {
  const isAllSelected = rows.length > 0 && selectedConflicts.length === rows.length;
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.length === 0 ? (
              <tr>
                <td colSpan={6} className="p-6 text-center text-sm text-slate-500">No phone conflicts to resolve.</td>
              </tr>
            ) : (
              rows.map((row) => {
                const isSelected = selectedConflicts.some((c) => c.key === row.key);
                return (
                  <tr key={row.key} className={`hover:bg-slate-50 transition ${isSelected ? 'bg-blue-50/10' : ''}`}>
                    <td className="p-4 text-center">
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => onSelectConflict(row)}
                        className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
                      />
                    </td>
                    ...
                    <td className="p-4 text-right whitespace-nowrap space-x-3">
                      <button
                        ...
                        className="text-amber-700 hover:text-amber-900 font-semibold text-sm cursor-pointer"
                      >
                        Resolve
                      </button>
                      <button
                        ...
                        className="text-red-600 hover:text-red-800 font-semibold text-sm disabled:opacity-50 cursor-pointer inline-flex items-center gap-1"
...
const RestoreManagement = ({ onConflictCountChange }) => {
  ...
  const handleSelectConflict = (row) => {
    setSelectedConflicts((prev) => {
      const exists = prev.some((c) => c.key === row.key);
      if (exists) {
        return prev.filter((c) => c.key !== row.key);
      } else {
        return [...prev, row];
      }
    });
  ...
  const handleSelectAllConflicts = () => {
    if (selectedConflicts.length === activeRows.length) {
      setSelectedConflicts([]);
    } else {
      setSelectedConflicts(activeRows);
    }
  };

  const handleBulkDeleteConflicts = async () => {
    if (selectedConflicts.length === 0) return;

    const ok = await confirm({
      title: 'Permanently Delete Selected Conflicts',
      message: `Are you sure you want to permanently delete the ${selectedConflicts.length} selected conflict records? This action cannot be undone.`,
    ...
    try {
      ...
      await Promise.all(
        selectedConflicts.map(async (row) => {
          try {
            const base = row.type === 'Customer' ? '/restore-management/customers' : '/restore-management/employees';
            await api.delete(`${base}/${row.backup._id}/permanent`);
            successCount++;
          } catch (err) {
            console.error(`Failed to delete permanent conflict for ${row.backup.name}`, err);
            failCount++;
          }
        })
      );
      ...
      if (failCount > 0) {
        ...
        setMessage(`Permanently deleted all ${successCount} selected conflict records successfully.`);
      }
      setSelectedConflicts([]);
      await fetchPreview();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedConflicts.length > 0 && (
        <div className="p-4 bg-red-50 border border-red-200 rounded-lg flex justify-between items-center animate-in slide-in-from-top-1 duration-150 shrink-0">
          <span className="text-sm font-bold text-red-800">
            {selectedConflicts.length} conflict(s) selected
          </span>
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={handleBulkDeleteConflicts}
      ...
      <ConflictTable
        ...
        onSelectAllConflicts={handleSelectAllConflicts}
      />
