    ...
    <div className="space-y-6">
      ...
      <div className="card overflow-hidden p-0 border border-slate-200">
        {loading ? (
           ...
           <div className="p-8 text-center text-slate-500">No customers found. Add your first customer!</div>
        ) : (
                  ...
                  <React.Fragment key={c._id}>
                    <tr className="hover:bg-slate-50 transition">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {vehicleCount > 0 && (
                            <button
                              ...
                              title={isExpanded ? 'Hide vehicles' : 'View all vehicles'}
                            >
                      ...
                      <td className="p-4 text-right space-x-3">
                        <button onClick={() => openBillsModal(c)} className="text-green-600 hover:text-green-800 font-medium text-sm">View Bills</button>
                        {canWrite && (
                          <>
                            <button onClick={() => handleEdit(c)} className="text-blue-600 hover:text-blue-800 font-medium text-sm">Edit</button>
                            <button onClick={() => handleDelete(c._id)} className="text-red-600 hover:text-red-800 font-medium text-sm">Delete</button>
                          </>
                        )}
                      </td>
                    </tr>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {vehicles.map((v, idx) => (
                              <span
                                ...
                                className="inline-flex items-center bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-mono font-semibold text-slate-800 shadow-sm"
                              >
                                {v.number}
                              </span>
                            ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(formData.vehicles || []).map((v, idx) => (
                    <span key={idx} className="inline-flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-full px-3 py-1 text-xs font-mono">
                      {v.number}
                      <button type="button" onClick={() => removeVehicleFromForm(idx)} className="text-red-500 hover:text-red-700">&times;</button>
                    </span>
                  ))}
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {selectedCustomerBills.bills.map((bill) => {
                      ...
                      <tr key={bill._id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4 text-slate-800 font-medium">{bill.vehicleNumber || '—'}</td>
                        ...
                        <td className="p-4"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${bill.paymentStatus === 'Paid' ? 'bg-green-100 text-green-800' : bill.paymentStatus === 'Partially Paid' ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800'}`}>{bill.paymentStatus}</span></td>
                        <td className="p-4 text-center">
                          {canWrite && bill.paymentStatus !== 'Paid' && (
                            <button onClick={() => openPaymentModal(bill)} className="text-blue-600 hover:text-blue-800 font-medium text-sm">Pay</button>
                          )}
                        </td>
                      </tr>
                    );
